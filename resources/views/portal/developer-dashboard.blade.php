...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .kpi.asignado { border-left: 4px solid var(--purple); }
        .kpi.pagado { border-left: 4px solid #059669; }
        ...
        .kpi.asignado .kpi-value { color: var(--purple); }
        .kpi.pagado .kpi-value { color: #059669; }
        ...
        .progress-fill { height: 100%; background: linear-gradient(90deg, var(--purple), #10b981); border-radius: 5px; transition: width 0.6s ease; }
        ...
        .mono.asignado { color: var(--purple); }
        .mono.pagado { color: #059669; }
        .mono.saldo { color: #dc3545; }
        .mono.mute { color: #bbb; }
        ...
        .mini-bar { height: 5px; background: #e2e8f0; border-radius: 3px; overflow: hidden; min-width: 60px; margin-top: 0.3rem; }
        .mini-bar-fill { height: 100%; background: var(--purple); border-radius: 3px; }
...
<div class="wrap">
    <div class="head">
        <div class="head-info">
            ...
            <div>
                <h1><i class="fas fa-laptop-code"></i> {{ $developer->nombre }}</h1>
                <div class="meta">
    ...
    <div class="kpi-grid">
        <div class="kpi asignado">
            <div class="kpi-label"><i class="fas fa-file-signature"></i> Asignado total</div>
            <div class="kpi-value">{{ $fmtCop($kpis['asignado_cop']) }}</div>
            <div class="kpi-sub">suma de lo pactado en todos tus proyectos (≈ COP)</div>
        </div>
        <div class="kpi pagado">
            <div class="kpi-label"><i class="fas fa-check-circle"></i> Pagado a ti</div>
            <div class="kpi-value">{{ $fmtCop($kpis['pagado_cop']) }}</div>
            <div class="kpi-sub">{{ $kpis['porcentaje'] }}% del total · {{ $payments->count() }} transferencias</div>
        </div>
        <div class="kpi saldo">
            <div class="kpi-label"><i class="fas fa-hourglass-half"></i> Pendiente por pagar</div>
            <div class="kpi-value">{{ $fmtCop($kpis['saldo_cop']) }}</div>
            <div class="kpi-sub">saldo a tu favor</div>
        </div>
    </div>

    @if($kpis['asignado_cop'] > 0)
    ...
    <div class="panel">
        <div class="panel-head">
            <h3><i class="fas fa-briefcase"></i> Tus proyectos</h3>
                    ...
                    <tbody>
                        @foreach($proyectosResumen as $proj)
                            <tr>
                                ...
                                <td style="color:#64748b; font-size:0.8rem;">{{ $proj['es_recurrente'] ? 'Recurrente' : ($proj['fecha_entrega'] ? $proj['fecha_entrega']->format('d/m/Y') : '—') }}</td>
                                <td class="mono asignado">{{ $fmtMoneda($proj['asignado'], $proj['moneda']) }}</td>
                                <td class="mono pagado">
                                    {{ $fmtMoneda($proj['pagado'], $proj['moneda']) }}
                                    <div class="mini-bar"><div class="mini-bar-fill" style="width: {{ min($proj['pct'], 100) }}%; background: {{ $proj['pct'] >= 100 ? '#10b981' : 'var(--purple)' }};"></div></div>
                                </td>
                                <td class="mono {{ $proj['saldo'] > 0 ? 'saldo' : 'mute' }}">{{ $fmtMoneda($proj['saldo'], $proj['moneda']) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        @else
            <div class="empty"><i class="fas fa-folder-open"></i><p>Aún no hay proyectos asignados.</p></div>
        @endif
    </div>
    ...
    <div class="panel">
        <div class="panel-head">
            ...
            <span class="muted">{{ $payments->count() }} transferencias</span>
        </div>
        @if($payments->count() > 0)
            <div style="overflow-x: auto;">
                <table>
                    ...
                    <tbody>
                        @foreach($payments as $pay)
                            <tr>
                                ...
                                <td class="mono pagado">+{{ $fmtMoneda($pay->monto, $pay->moneda ?? 'COP') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
