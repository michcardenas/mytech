...
<style>
    ...
    .bn-hero p { font-size:.82rem; opacity:.8; margin:.2rem 0 0; }
    .bn-hero .icon { display:inline-flex; width:36px; height:36px; border-radius:10px; background:rgba(255,255,255,.18); align-items:center; justify-content:center; color:#fff; }
    .bn-btn { display:inline-flex; align-items:center; gap:.4rem; padding:.55rem 1rem; border-radius:10px; font-weight:600; font-size:.83rem; text-decoration:none; border:1px solid rgba(255,255,255,.18); background:rgba(255,255,255,.1); color:#EFF6FF; }
    .bn-btn:hover { background:rgba(255,255,255,.18); color:#fff; }
...
<div class="bn-wrap">
    <div class="bn-hero">
        <div>
            <h1><span class="icon"><i class="fas fa-store"></i></span> Banners para clientes</h1>
            <p>Ofrécele servicios a tus clientes. Se muestran al entrar a su portal (Mis proyectos).</p>
        </div>
        <a href="{{ route('admin.banners.index') }}" class="bn-btn"><i class="fas fa-bullhorn"></i> Banners de comerciales</a>
    </div>
    ...
    <div class="bn-card">
        ...
        <form method="POST" action="{{ route('admin.cliente-banners.store') }}" enctype="multipart/form-data" class="bn-form">
            @csrf
            <div class="bn-fld full">
                <label>Título *</label>
                <input type="text" name="titulo" required maxlength="150" placeholder="Bolsas de horas prepagadas: ahorra en tus próximos cambios">
            </div>
            <div class="bn-fld full">
                <label>Mensaje</label>
                <textarea name="mensaje" maxlength="600" placeholder="Reserva horas por adelantado y paga menos por hora. Ideal para las mejoras que vengan en tu proyecto."></textarea>
            </div>
            <div class="bn-fld">
            ...
            <div class="bn-fld">
                <label>Color</label>
                <select name="color">
                    @foreach(\App\Models\ClienteBanner::COLORES as $k => $c)
            ...
            <div class="bn-fld">
                ...
                <input type="text" name="cta_texto" maxlength="60" placeholder="Ver las bolsas de horas">
            </div>
            <div class="bn-fld">
                <label>URL del botón</label>
                <input type="url" name="cta_url" maxlength="500" placeholder="https://mytechsolutionsco.com/bolsas-de-horas">
            </div>
            <div class="bn-fld">
                <label>Desde</label>
    ...
    <div class="bn-card">
        ...
        @forelse($banners as $b)
            <div class="bn-item">
                ...
                <div class="bn-meta">
                    ...
                    <span class="pill dest">{{ $b->client?->nombre ?? 'Todos los clientes' }}</span>
                    <span>
                        {{ $b->desde?->format('d/m/Y') ?? 'sin inicio' }} → {{ $b->hasta?->format('d/m/Y') ?? 'sin fin' }}
                        · orden {{ $b->orden }}
                    </span>
                    <span class="spacer">
                        <form method="POST" action="{{ route('admin.cliente-banners.toggle', $b) }}">
                            @csrf @method('PUT')
                        ...
                        <form method="POST" action="{{ route('admin.cliente-banners.destroy', $b) }}" onsubmit="return confirm('¿Eliminar este banner?');">
                            @csrf @method('DELETE')
                ...
                <div id="edit-{{ $b->id }}" class="bn-edit-form d-none">
                    <form method="POST" action="{{ route('admin.cliente-banners.update', $b) }}" enctype="multipart/form-data" class="bn-form">
                        @csrf @method('PUT')
                        <div class="bn-fld full">
                            <label>Título *</label>
                        ...
                        <div class="bn-fld">
                            ...
                            <select name="client_id">
                                <option value="">Todos los clientes</option>
                                @foreach($clientes as $c)
                                    <option value="{{ $c->id }}" {{ $b->client_id === $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="bn-fld">
        ...
        @empty
            <div style="text-align:center; padding:2.5rem; color:#94A3B8;">
                <i class="fas fa-store" style="font-size:2rem; opacity:.4;"></i>
                <p style="margin-top:.5rem;">Aún no hay banners para clientes. Crea el primero arriba.</p>
            </div>
        @endforelse
    </div>
</div>
