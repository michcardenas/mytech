        ...
        <div class="max-w-4xl">
            ...
            <div data-animate>
                <span class="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border border-mt-accent-line bg-mt-accent-soft text-mt-accent font-mono text-[11px] uppercase tracking-[0.18em]">
                    ...
                    {{ $homeContent['hero_badge'] ?? 'Software a medida' }}
                </span>
            </div>
            ...
            <div class="mt-11 flex flex-wrap gap-3.5" data-animate>
                <a href="{{ route('contacto.index') }}" class="mt-btn-primary">
                    {{ $homeContent['hero_button_text'] ?? 'Cotizar mi proyecto' }}
                    <span aria-hidden="true">→</span>
                </a>
                @php
                    $waNumber  = $homeContent['hero_whatsapp_number']  ?? '573337246403';
                    $waMessage = $homeContent['hero_whatsapp_message'] ?? 'Hola, me interesa conocer más sobre sus servicios';
                    $waText    = $homeContent['hero_whatsapp_text']    ?? 'Hablemos por WhatsApp';
            ...
            @if(count($stats) > 0)
                <div class="mt-12 pt-8 border-t border-mt-border max-w-2xl
                    ...
                    @foreach($stats as $stat)
                        @php
                            // Si el valor empieza con "+", lo guardamos como prefijo;
                            // si es numérico puro, lo animamos con counter.
                            $raw       = trim($stat['value']);
                            $hasPlus   = str_starts_with($raw, '+');
                            $numeric   = (int) preg_replace('/[^0-9]/', '', $raw);
                            $animatable = $numeric > 0;
                        @endphp
                        <div>
                            <div class="text-3xl md:text-4xl font-display font-semibold text-mt-text leading-none tracking-tight flex items-baseline">
                                @if($hasPlus)<span aria-hidden="true">+</span>@endif
                                @if($animatable)
                                    <span data-counter="{{ $numeric }}" data-counter-decimals="0" aria-label="{{ $stat['value'] }}">0</span>
                                @else
